export const cguContent = `
1. Objet

Les présentes Conditions Générales d’Utilisation définissent les conditions d’accès et d’utilisation de l’application ConteMagiqueIA et de ses services associés.

En utilisant l’application, l’utilisateur reconnaît avoir pris connaissance des présentes conditions et les accepter.


2. Présentation du service

ConteMagiqueIA permet notamment de :

• créer des histoires personnalisées ;
• générer des illustrations ;
• écouter une narration audio ;
• sauvegarder certaines histoires ;
• utiliser différents carnets selon les droits disponibles ;
• accéder à des fonctionnalités complémentaires proposées dans l’application.

Certaines fonctionnalités peuvent évoluer, être ajoutées, modifiées ou supprimées au fil des versions.


3. Accès à l’application

L’accès à certaines fonctionnalités peut nécessiter :

• un appareil compatible ;
• une connexion Internet ;
• un compte utilisateur ;
• une version récente de l’application.

ConteMagiqueIA ne garantit pas la compatibilité avec tous les appareils ou toutes les versions de systèmes d’exploitation.


4. Création d’un compte

Certaines fonctionnalités nécessitent la création d’un compte.

L’utilisateur s’engage à fournir des informations exactes et à maintenir ses informations à jour.

Il est responsable de la confidentialité de ses identifiants et de l’utilisation de son compte.

Toute utilisation réalisée depuis son compte est présumée avoir été effectuée par lui ou avec son autorisation, sauf signalement contraire.


5. Utilisation par les enfants

ConteMagiqueIA est principalement destinée à un usage familial.

Lorsqu’un enfant utilise l’application, cette utilisation doit être réalisée sous la responsabilité d’un parent ou représentant légal lorsque cela est nécessaire.

Les parents sont invités à accompagner les enfants dans l’utilisation de l’application et à éviter la saisie de données personnelles sensibles.


6. Carnets et droits d’utilisation

ConteMagiqueIA peut proposer différents carnets donnant accès à un nombre déterminé d’histoires.

À titre d’exemple :

• Carnet Texte ;
• Carnet Illustré.

Le nombre d’histoires disponibles dépend de l’offre souscrite ou du nombre de crédits restant sur le compte.

Lorsque plusieurs carnets sont disponibles, l’utilisateur peut choisir le carnet à utiliser avant la génération de son histoire.


7. Carnet Texte

Le Carnet Texte permet de générer des histoires sans illustrations générées pour chaque scène.

Selon les fonctionnalités disponibles, la narration audio, la sauvegarde et les modes de lecture peuvent rester accessibles.


8. Carnet Illustré

Le Carnet Illustré permet de générer des histoires accompagnées d’illustrations créées automatiquement.

Le nombre, le style et le rendu des illustrations peuvent varier en fonction de l’histoire, des paramètres choisis et des services techniques utilisés.


9. Décompte des histoires

Lorsqu’une histoire est générée avec succès, une unité peut être déduite du carnet sélectionné.

En cas d’échec technique empêchant la génération complète, ConteMagiqueIA peut mettre en place des mécanismes permettant d’éviter un décompte injustifié lorsque cela est techniquement possible.


10. Intelligence artificielle

ConteMagiqueIA utilise des technologies d’intelligence artificielle pour générer certains contenus.

Les histoires, illustrations ou narrations peuvent donc comporter :

• des variations ;
• des imprécisions ;
• des incohérences ;
• des résultats différents pour une même demande.

Les contenus générés sont destinés principalement à un usage créatif et récréatif.


11. Absence de garantie sur les contenus générés

ConteMagiqueIA ne garantit pas :

• l’exactitude absolue des contenus générés ;
• leur caractère unique ;
• l’absence totale d’erreur ;
• leur adéquation à une attente particulière.

L’utilisateur reste responsable de la manière dont il utilise ou partage les contenus générés.


12. Contenus interdits

Il est interdit d’utiliser ConteMagiqueIA afin de générer, promouvoir ou diffuser des contenus manifestement :

• illégaux ;
• haineux ;
• discriminatoires ;
• pornographiques ;
• sexuellement explicites impliquant des mineurs ;
• destinés à faciliter une activité criminelle ;
• portant atteinte aux droits d’un tiers ;
• visant à harceler, menacer ou intimider une personne ;
• destinés à contourner les systèmes de sécurité de l’application.


13. Modération

ConteMagiqueIA peut utiliser des systèmes automatisés ou des règles techniques afin de limiter certaines demandes.

Certaines demandes peuvent être refusées lorsqu’elles présentent un risque de non-conformité, de danger ou d’abus.


14. Suspension d’un compte

ConteMagiqueIA peut suspendre temporairement ou définitivement l’accès à un compte notamment en cas :

• d’utilisation frauduleuse ;
• de tentative de piratage ;
• d’abus manifeste du service ;
• de violation grave ou répétée des présentes CGU ;
• d’atteinte à la sécurité de l’application ;
• d’obligation légale.

Lorsque cela est possible et approprié, l’utilisateur peut être informé du motif de la suspension.


15. Sauvegarde des histoires

Certaines histoires peuvent être sauvegardées dans l’application ou dans des services cloud associés.

ConteMagiqueIA met en œuvre des moyens raisonnables pour assurer la disponibilité des contenus, mais ne garantit pas une conservation permanente en toutes circonstances.

L’utilisateur est invité à conserver une copie des contenus auxquels il attache une importance particulière lorsque cette possibilité est disponible.


16. Fonctionnalités audio

Certaines histoires peuvent être lues grâce à des technologies de synthèse vocale.

La qualité, la disponibilité, le timbre et la prononciation peuvent varier selon les prestataires utilisés et la langue du contenu.


17. Disponibilité du service

ConteMagiqueIA s’efforce de maintenir l’application accessible dans de bonnes conditions.

Toutefois, des interruptions peuvent intervenir notamment pour :

• maintenance ;
• mise à jour ;
• incident technique ;
• évolution de l’infrastructure ;
• indisponibilité d’un prestataire ;
• raison de sécurité ;
• obligation légale.

Ces interruptions ne donnent pas automatiquement droit à indemnisation.


18. Services tiers

Le fonctionnement de ConteMagiqueIA peut dépendre de prestataires externes, notamment pour :

• l’hébergement ;
• Firebase ;
• les services d’intelligence artificielle ;
• le stockage ;
• la narration audio ;
• Google Play ;
• l’App Store ;
• les paiements.

ConteMagiqueIA ne peut garantir la disponibilité permanente de services exploités par des tiers.


19. Achats

Lorsque les paiements réels sont activés, les achats sont effectués via les plateformes ou prestataires autorisés.

Les conditions applicables aux carnets payants sont détaillées dans les Conditions Premium disponibles dans le Centre juridique.


20. Prix

Les prix applicables sont ceux affichés au moment de l’achat.

Ils peuvent évoluer.

Toute modification de prix ne remet pas en cause les achats déjà définitivement réalisés, sauf disposition légale contraire.


21. Propriété intellectuelle de l’application

ConteMagiqueIA, son code, son architecture, son interface, son identité visuelle, son logo, ses éléments graphiques, ses textes propriétaires et ses fonctionnalités sont protégés par les règles applicables en matière de propriété intellectuelle.

L’utilisation de l’application ne transfère aucun droit de propriété sur ces éléments à l’utilisateur.


22. Licence d’utilisation

ConteMagiqueIA accorde à l’utilisateur un droit personnel, limité, non exclusif et non cessible d’utiliser l’application conformément à sa destination.

Les règles détaillées relatives à cette licence figurent dans le Contrat de licence utilisateur disponible dans le Centre juridique.


23. Restrictions techniques

Sauf lorsque la loi l’autorise expressément, il est interdit de :

• copier le logiciel ;
• modifier le code ;
• désassembler l’application ;
• décompiler l’application ;
• pratiquer de l’ingénierie inverse ;
• contourner les protections techniques ;
• automatiser abusivement les requêtes ;
• exploiter les services afin de créer un service concurrent.


24. Contenus créés par l’utilisateur

L’utilisateur reste responsable des informations, textes et instructions qu’il fournit à l’application.

Il s’engage à ne pas transmettre de contenu dont il ne possède pas les droits nécessaires lorsque cette transmission serait illicite.


25. Contenus générés

Les droits pouvant s’appliquer aux histoires, images ou autres contenus générés dépendent notamment :

• de la législation applicable ;
• des technologies utilisées ;
• des conditions des fournisseurs concernés ;
• de la nature du contenu.

L’utilisateur bénéficie au minimum du droit d’utiliser les contenus générés dans les limites prévues par les présentes CGU et par les règles applicables.


26. Utilisation personnelle

Sauf indication contraire, les contenus générés sont destinés principalement à une utilisation personnelle et familiale.

Toute exploitation commerciale importante peut nécessiter une vérification préalable des droits applicables.


27. Partage

Lorsque la fonctionnalité de partage est disponible, l’utilisateur peut partager certains contenus sous sa propre responsabilité.

Il lui appartient de vérifier qu’aucune donnée personnelle sensible ou information confidentielle n’est incluse dans le contenu partagé.


28. Protection des données personnelles

Les traitements de données personnelles réalisés dans le cadre de ConteMagiqueIA sont décrits dans la Politique de confidentialité accessible depuis le Centre juridique.


29. Suppression du compte

L’utilisateur peut demander la suppression de son compte depuis les fonctions prévues dans l’application.

Cette suppression peut entraîner la perte définitive de certaines données et histoires associées au compte.


30. Sécurité

L’utilisateur s’engage à ne pas tenter :

• d’accéder à des comptes tiers ;
• de compromettre la sécurité du service ;
• d’introduire du code malveillant ;
• de contourner les limites techniques ;
• de perturber volontairement le fonctionnement du service.


31. Responsabilité de l’utilisateur

L’utilisateur est responsable de son utilisation de l’application et des contenus qu’il demande, utilise ou partage.

Il lui appartient de respecter les lois applicables ainsi que les droits des tiers.


32. Responsabilité de ConteMagiqueIA

ConteMagiqueIA met en œuvre des moyens raisonnables afin d’assurer le fonctionnement de ses services.

Toutefois, sauf disposition légale impérative, ConteMagiqueIA ne peut être tenue responsable de dommages résultant notamment :

• d’une utilisation non conforme ;
• d’une indisponibilité temporaire ;
• d’un appareil incompatible ;
• d’un problème provenant d’un service tiers ;
• d’un contenu généré automatiquement ;
• de la perte d’informations lorsque celle-ci ne résulte pas d’une faute engageant sa responsabilité.


33. Force majeure

ConteMagiqueIA ne pourra être tenue responsable d’un retard ou d’une inexécution résultant d’un événement présentant les caractéristiques de la force majeure au sens du droit applicable.


34. Mise à jour de l’application

ConteMagiqueIA peut proposer ou imposer certaines mises à jour nécessaires au maintien de la sécurité ou au bon fonctionnement du service.

Une ancienne version peut ne plus être compatible avec les services en ligne.


35. Évolution des fonctionnalités

ConteMagiqueIA peut faire évoluer son application afin notamment :

• d’améliorer l’expérience utilisateur ;
• de corriger des bugs ;
• d’ajouter de nouvelles fonctions ;
• de supprimer des fonctions obsolètes ;
• de renforcer la sécurité ;
• de respecter de nouvelles obligations légales.


36. Modification des CGU

Les présentes CGU peuvent être mises à jour.

Lorsque les modifications sont importantes et que la réglementation l’exige, l’utilisateur peut être informé par un moyen approprié.

La version applicable est celle publiée dans l’application à la date de consultation.


37. Nullité partielle

Si une disposition des présentes CGU est déclarée invalide ou inapplicable, les autres dispositions restent applicables.


38. Absence de renonciation

Le fait pour ConteMagiqueIA de ne pas appliquer immédiatement une disposition des présentes CGU ne constitue pas une renonciation à l’appliquer ultérieurement.


39. Droit applicable

Les présentes CGU sont régies par le droit français, sous réserve des règles impératives éventuellement applicables au consommateur dans son pays de résidence.


40. Litiges

En cas de difficulté, l’utilisateur est invité à contacter ConteMagiqueIA afin de rechercher une solution amiable.

Les règles relatives à la compétence juridictionnelle et aux droits des consommateurs restent applicables conformément à la législation en vigueur.


41. Contact

Pour toute question relative aux présentes Conditions Générales d’Utilisation :

ConteMagiqueIA

E-mail :
contact@contemagiqueia.fr
`;